    ...
    if user in users:
        ...
        if bcrypt.checkpw(password.encode('utf-8'), stored_hashed_password.encode('utf-8')):
            if remember_me.get() == True:
                with open(os.path.join(os.getcwd(), r"rememberMe.txt"), "w") as rememberMe_file:
...
def login_window():
    ...
    w_login = ttk.Window()
    w_login.title("Log In")
    w_login.geometry('250x360+1030+360')
    style = ttk.Style()
    style.theme_use("cyborg")
    ttk.Label(w_login, text="Log In", font="Calibri 24 bold").pack(pady=20)
    ...
    ttk.Label(master=f_login,text="User Name", anchor="w", justify="left",width=22).pack()
    ...
    f_remember_me.pack()
    ttk.Button(master=f_login, text="Log In", command= lambda: [check_credentials(user_name.get(), user_password.get()), update_current_user(user_name.get())]).pack(padx=5, pady=5)
    ttk.Button(master=f_login, text="Sign Up", command= lambda:[w_login.destroy(),signup_window()]).pack(padx=5, pady=5)
    f_login.pack()
    w_login.mainloop()
...
def signup_window():
    w_signup = ttk.Window()
    w_signup.title("Sign Up")
    w_signup.geometry("250x360+1030+360")
    style = ttk.Style()
    style.theme_use("cyborg")
    ttk.Label(w_signup, text="Sign Up", font="Calibri 24 bold").pack(pady=20)
    ...
    ttk.Label(master=f_signup,text="User Name", anchor="w", justify="left",width=22).pack()
    ttk.Entry(master=f_signup,justify="left",width=20,textvariable=user_name_su).pack(padx=10,pady=(2, 10))
    ttk.Label(master=f_signup,text="Password", anchor="w", justify="left",width=22).pack()
    ttk.Entry(master=f_signup,justify="left",width=20,textvariable=user_password_su, show="*").pack(padx=10,pady=(2, 10))
    ttk.Label(master=f_signup,text="Repeat Password", anchor="w", justify="left",width=22).pack()
    ttk.Entry(master=f_signup,justify="left",width=20,textvariable=user_password_reentry, show="*").pack(padx=10,pady=(2, 10))
    ttk.Button(master=f_signup, text="Sign Up", command= lambda: add_new_user(user_name_su.get(), user_password_su.get(), user_password_reentry.get(), w_signup)).pack(padx=5, pady=5)
    ttk.Button(master=f_signup, text="Back", command= lambda:[w_signup.destroy(), login_window()]).pack(padx=5, pady=5)
    f_signup.pack()
    w_signup.mainloop()
...
def create_challenge(frame, x, operator, y):
    ...
    tk.Label(frame, text="Timer: ").pack(side="right")
...
def chalanges(c_frame):
    clear_frame(c_frame)
    button_frame = ttk.Frame(c_frame)
...
def logout(appwindow):
    ...
    if answer:
        with open(os.path.join(os.getcwd(), r"rememberMe.txt"), "r") as rememberMe_file:
            lines = rememberMe_file.readlines()
        if lines:
            lines.pop(0)
            with open(os.path.join(os.getcwd(), r"rememberMe.txt"), 'w') as file:
